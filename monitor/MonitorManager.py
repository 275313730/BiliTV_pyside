...
class MonitorManager(QObject):
    ...
    def get_new_data(self, uid: int):
        ...
        if DataManager.need_update(uid, "dynamic"):
            ...
            if self.system == "windows" and update_status:
                wintoaster = WindowsToaster('BiliTV')
                new_toast = ToastText1()
                new_toast.SetBody(f'「{nick_name}」发布了一条新动态')
        ...
        if DataManager.need_update(uid, "video"):
            ...
            if self.system == "windows" and update_status:
                wintoaster = WindowsToaster('BiliTV')
                new_toast = ToastText1()
                new_toast.SetBody(f'「{nick_name}」发布了一条新视频')
                wintoaster.show_toast(new_toast)
        else:
        ...
        if DataManager.need_update(uid, "live"):
            ...
            if self.system == "windows" and update_status:
                ...
                if new_live_info['live_status']:
                    new_toast.SetBody(f'「{nick_name}」上播了')
                else:
                    new_toast.SetBody(f'「{nick_name}」下播了')
                wintoaster.show_toast(new_toast)
        else:
